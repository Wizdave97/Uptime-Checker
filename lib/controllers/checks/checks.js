                    ...
                    __data.read('users',userPhone).then(({data})=>{
                        ...
                        if(typeof data =='object'){
                            ...
                            if(userChecks && userChecks.length<config.maxChecks){
                                //Verify the url and ensure it has dns entries
                                let parsedUrl=url.parse(payload.protocol+'://'+payload.url,true)
                                ...
                                dns.resolve(hostName,function(err,records){
                                    ...
                                    else reject({status:402,error:'The hostname did not resolve on any dns entries'})
                                })
